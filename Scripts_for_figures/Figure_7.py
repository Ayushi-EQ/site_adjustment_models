...
for cluster_id in range(n_clusters):
    stats = res_df.index[cluster_labels==cluster_id+1]
...
df_site = pd.merge(stations[['stat_id','stat_name','vs30','Geom','Z1_NZVM','Z1_Meas','T0','Slope','H250','H1250']],df_latlong,on='stat_name')
...
culpritpath = r"C:\Users\ati47\OneDrive - University of Canterbury\Desktop\PhD\10. Research\Objective 1\1. Residuals\2. Interpretations"
...
# colors = ['#e6194B', '#808000', '#f032e6', '#4363d8', 'r', 'b', 'y', 'g', 'brown']
colors = ['0.1', '0.25', '0.4', '0.55']
num_rows = (n_clusters + 2) // 3  
num_cols = 2
fig, axes = plt.subplots(num_rows, num_cols, figsize = (15.43, 11.34), constrained_layout=True, sharex=True, sharey=True)

fig.text(0.0, 1, '(a)', fontsize=40, fontweight='bold', ha='left', va='top')
if num_rows == 1:
...
grouped_dataerr = sim_sitereserr.groupby(cluster_labels)
...
# Define the remapping of original cluster IDs to new labels
cluster_remap = {3: 1, 4: 2, 1: 3, 2: 4}  # Original ID to new ID

# Get all cluster data first
all_clusters = [group for _, group in grouped_data]

# Plot in the desired order with remapped labels
for idx, (cluster_idx, ax) in enumerate(zip([2, 3, 0, 1], axes.flatten()[:n_clusters])):
    cluster_data = all_clusters[cluster_idx]
    original_id = cluster_idx + 1  # Original cluster ID (1, 2, 3, 4)
    new_id = cluster_remap[original_id]  # Remapped cluster ID (1, 2, 3, 4)

    # Plot the cluster's data in the current subplot
    ax.semilogx(cluster_data.columns, cluster_data.values.T, linewidth=1, color='#a9a9a9', alpha=0.4, picker=True, pickradius=5, label=sorted(stat_names[original_id-1]))
    ax.semilogx(cluster_data.columns, cluster_mean.iloc[original_id-1,:], color=colors[new_id-1], linewidth=2)
    ax.semilogx(cluster_data.columns, cluster_mean.iloc[original_id-1,:]-cat_phiS2S.iloc[original_id-1,:], color=colors[new_id-1], linestyle='--', linewidth=2)
    ax.semilogx(cluster_data.columns, cluster_mean.iloc[original_id-1,:]+cat_phiS2S.iloc[original_id-1,:], color=colors[new_id-1], linestyle='--', linewidth=2)
    ax.set_xlim([0.01, 10])
    ax.set_ylim([-1.5, 1.5])
    ax.axhline(0, color='k', linestyle='--')
    if new_id in [1, 3]:  # Adjust ylabel for the first column
        ax.set_ylabel('${\it \delta S2S_s}$', size=40)
    ax.tick_params(labelsize=40, direction='in', axis='both', which='both')
    ax.text(0.0105, 1.2, f"Cluster {new_id} (N = {len(cluster_data)})", fontsize=40, fontweight='bold')
    ax.grid(color='gray', linestyle='dashed', which='both', linewidth=0.4,alpha=0.5)

# Add underprediction/overprediction labels
axes[0][0].text(0.15, 0.75, 'Underprediction', size=40, fontstyle='italic')
axes[0][0].text(0.15, -1.2, 'Overprediction', size=40, fontstyle='italic')
for ax in axes[1]:
...
ax_edge.annotate(
    "Cluster mean",
    xy=(0.38, 0.27),       # point on the solid line
    xytext=(0.1, 0.9),      # text position
    fontsize=40,
...
ax_edge.annotate(
    ...
    xy=(0.30, -0.02),       # point on upper dashed line
    xytext=(0.3, -0.75),      # text position
    fontsize=40,
    arrowprops=dict(arrowstyle="->", lw=1, color="black"),
    ha="center", va="center"
)
axes[1][0].tick_params(axis='x', pad=20)
axes[1][1].tick_params(axis='x', pad=20)
plt.savefig('Clusters_WholeNZ.pdf')

#%% Plot Histograms - Paper 1 - WE ARE USING THIS
bin_labels = ['Hill','Valley','Basin Edge','Basin']
grouped_data = df_site.groupby('Cluster')
selected_clusters = [3, 4, 1, 2]
colors = ['#993333','#9370DB','#66C266','#1E90FF'] 
alpha=1
bin_counts = []
for cluster_id in selected_clusters:
    ...
    bin_counts.append(cluster_counts)

bin_counts = np.array(bin_counts)  # Shape: (num_clusters, num_geomorphologies)

bar_width = 0.2  # Adjust for spacing
fig, ax = plt.subplots(figsize=(10.09,  8.1), constrained_layout=True)
fig.text(0.0, 1, '(b)', fontsize=30, fontweight='bold', ha='left', va='top')

x_positions = np.arange(len(selected_clusters))  # One position per cluster

for i, bin_label in enumerate(bin_labels):
    ax.bar(x_positions + i * bar_width, bin_counts[:, i], bar_width, label=bin_label, color=colors[i],alpha=alpha,edgecolor='k')

ax.set_xlabel('Clusters', size=30)
ax.set_ylabel('Number of sites in each cluster', size=30)
ax.tick_params(labelsize=30)
ax.legend(loc='upper left',fontsize=30)
ax.set_xticks(x_positions + (bar_width * (len(bin_labels) - 1) / 2))
ax.set_xticklabels([f"Cluster {i+1}" for i in range(len(selected_clusters))])
plt.savefig('Interpretation of NZ clusters.pdf')
#%% 3rd figure - Geom and Clusters std
colors = ['0.1', '0.25', '0.4', '0.55']
...
grouped_data_geom = res_df.groupby(Features[category])
grouped_dataerr_geom = sim_sitereserr.groupby(Features[category])

cat_phiS2S_geom = {}

for label_geom, group_errdS2S_geom in grouped_dataerr_geom:
    group_dS2S_geom = grouped_data_geom.get_group(label_geom)
    Cat_std_geom = np.sqrt(np.mean((group_dS2S_geom - np.mean(group_dS2S_geom, axis=0))**2, axis=0) + np.mean((group_errdS2S_geom)**2, axis=0))
    cat_phiS2S_geom[label_geom] = Cat_std_geom

cat_phiS2S_geom = pd.DataFrame(cat_phiS2S_geom)
cat_phiS2S_geom = cat_phiS2S_geom.transpose()

cat_stdall = np.sqrt(np.mean((res_df - np.mean(res_df, axis=0))**2, axis=0) + np.mean(sim_sitereserr**2, axis=0))
if category == "Geomorphology":
    desired_order = ['Basin', 'Basin Edge', 'Valley', 'Hill']
    grouped_data_geom = sorted(grouped_data_geom, key=lambda x: desired_order.index(x[0]))
    colors_geom = ['#1E90FF', '#66C266', '#9370DB', '#993333']

fig, ax = plt.subplots(figsize=(13.04, 8.35), constrained_layout=True)
fig.text(0.0, 1, '(c)', fontsize=20, fontweight='bold', ha='left', va='top')

# Plot geomorphic categories first
geo_lines = []
geo_labels = []
ax.semilogx(res_df.columns, cat_stdall, color='k', label='All sites', linewidth=3)
geo_lines.append(ax.get_lines()[-1])
geo_labels.append('All sites')
j = 0
for (cat_id, cat_data) in grouped_data_geom:
    ax.semilogx(res_df.columns, cat_phiS2S_geom.loc[cat_id], color=colors_geom[j], linewidth=3, label=f'{cat_id}')
    geo_lines.append(ax.get_lines()[-1])
    geo_labels.append(f'{cat_id}')
    j += 1

# Plot clusters separately
cluster_lines = []
cluster_labels = []
for i, original_id in enumerate(selected_clusters):
    new_id = i + 1  # New label (1, 2, 3, 4) based on order
    ax.semilogx(res_df.columns, cat_phiS2S.iloc[original_id-1, :], color=colors[new_id-1], linestyle='--', linewidth=2, label=f'Cluster {new_id}')
    cluster_lines.append(ax.get_lines()[-1])
    cluster_labels.append(f'Cluster {new_id}')
ax.annotate(
    'Range of clusters 1–4',
    xy=(0.27, 0.298),         # arrow tip (mid dashed cluster range)
    xytext=(0.2495,0.23),     # label text position below the curves
    textcoords='data',
...
ax.set_xlim([0.01, 10])
# all_lines = geo_lines + cluster_lines
# all_labels = geo_labels + cluster_labels
all_lines = geo_lines
all_labels = geo_labels
ax.legend(all_lines, all_labels, loc='best', ncol=1, fontsize=25, frameon=True)
ax.tick_params(axis='x', pad=10)
ax.set_xlabel('Vibration Period, T (s)', size=26)
ax.set_ylabel('Category standard deviation, $\phi_{S2S,cat}$', size=26)
ax.tick_params(labelbottom=True, labelleft=True, direction='in', axis='both', which='both', labelsize=22)
ax.grid(color='gray', linestyle='dashed', which='both', linewidth=0.4,alpha=0.5)
plt.savefig('Target phiS2S reduction.pdf')
#%% Plot Histograms - Paper 1 - If you wannt Geomorphology on x-axis
bin_labels = ['Hill','Valley','Basin-edge','Basin']
grouped_data = df_site.groupby('Cluster')
selected_clusters = [1,2,3,4]
colors = ['#e6194B','#808000','#f032e6','#4363d8']

cluster_percentages = []
for bin_label in bin_labels:
    total_sites = np.sum(df_site['Geom'] == bin_label)  # Total sites for this geomorphology
    percentages = []
    for cluster_id in selected_clusters:
        group = grouped_data.get_group(cluster_id)
        count = np.sum(group['Geom'] == bin_label)
        percentage = count / total_sites if total_sites > 0 else 0
        percentages.append(percentage)
    cluster_percentages.append(percentages)
cluster_percentages = np.array(cluster_percentages)  # Shape: (num_geomorph, num_clusters)

bin_percentages_by_geom = np.array(cluster_percentages)  # (num_geomorph, num_clusters)

bar_width = 0.2
fig, ax = plt.subplots(figsize=(7.29, 4.92), constrained_layout=True)
fig.text(0.0, 1, '(b)', fontsize=16, fontweight='bold', ha='left', va='top')

x_positions = np.arange(len(bin_labels))  # One position per geomorphology

for i, cluster_id in enumerate(selected_clusters):
    ax.bar(x_positions + i * bar_width, bin_percentages_by_geom[:, i], bar_width,
           label=f"Cluster {cluster_id}", color=colors[i])

ax.set_xlabel('Geomorphology', size=18)
ax.set_ylabel('Percentage of sites in each category (%)', size=18)
ax.tick_params(labelsize=16)
ax.set_xticks(x_positions + (bar_width * (len(selected_clusters) - 1) / 2))
